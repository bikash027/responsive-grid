...
// }
const lineSegments = [];
for(let x = minLength; x < maxLength; x += 20){
    lineSegments.push([
        {x, y: minLength},
        {x, y: maxLength}
    ])
}
for(let y = minLength; y < maxLength; y += 20){
    lineSegments.push([
        {x: minLength, y},
        {x: maxLength, y}
    ])
...
for(const lineSegment of lineSegments){
    ...
    newPath.setAttribute('filter', 'url(#blurred2)');
    // newPath.setAttribute('style', `stroke: ${Math.random()-0.5 < 0? white: pink}; filter: url(#blurred2);`)
